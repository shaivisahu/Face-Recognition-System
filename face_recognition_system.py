...
class TrainableFaceRecognizer:
    ...
    def predict(self, face_image):
        ...
        if self.model_type == "lbph":
            # LBPH: lower distance = better match; 0 is perfect, ~100 is poor
            confidence_pct = max(0.0, 100.0 - confidence) if confidence < 100 else 0.0
        else:
            # Eigenfaces/Fisherfaces: raw distance (lower = better, can be thousands)
            # Convert to 0-100 scale: treat 0 as 100% and clip at a reasonable max distance
            _MAX_DIST = 5000.0
            confidence_pct = max(0.0, 100.0 * (1.0 - min(confidence, _MAX_DIST) / _MAX_DIST))
